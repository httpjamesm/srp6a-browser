...
export class SRP {
   ...
   */
  computePrivateKey(username: string, password: Buffer, salt: Buffer): Buffer {
   ...
   * @param salt User's salt
   */
  computeVerifier(username: string, password: Buffer, salt: Buffer): Buffer {
    let x = this._engine.computeX(username, password, salt);
  ...
  generateClientEphemeralKey(): SRPKeyPair {
    ...
    const publicKey = this._engine
      .computeA(BigInt.fromBuffer(secretKey))
      .toBuffer();
    return {
      secretKey,
      publicKey,
  ...
  generateServerEphemeralKey(verifier: Buffer): SRPKeyPair {
    const secretKey = Buffer.from(randomBytes(this._engine.Nbytes));
    const publicKey = this._engine
      .computeB(BigInt.fromBuffer(secretKey), BigInt.fromBuffer(verifier))
      .toBuffer();
    return {
      secretKey,
      publicKey,
